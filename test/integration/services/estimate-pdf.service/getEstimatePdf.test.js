...
function buildService() {
  ...
  const customerEstimateItemRepository = Object.create(CustomerEstimateItemRepository.prototype);
  customerEstimateItemRepository.model = models.CustomerEstimateItem;
  ...
  customerEstimateService.customerEstimateItemRepository = customerEstimateItemRepository;
...
describe('EstimatePdfService.getEstimatePdf (integration)', () => {
  it('resolves a valid PDF buffer for a sent estimate owned by the customer', async () => {
    await seedWithTransaction(baseFixtures, async () => {
      ...
      const before = await models.CustomerEstimateItem.schema(TEST_SCHEMA).findByPk(estimateItemA.id);
      ...
      const after = await models.CustomerEstimateItem.schema(TEST_SCHEMA).findByPk(estimateItemA.id);
